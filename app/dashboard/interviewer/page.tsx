...
import { Users, FileText, Clock, CalendarIcon, Search, Bell } from 'lucide-react';
...
export default function InterviewerDashboard() {
  ...
  const [recentFeedback, setRecentFeedback] = useState<RecentFeedback[]>([
    { id: '1', candidateName: 'Sarah Johnson', position: 'Senior Software Engineer', recommendation: 'Strong Hire' },
    { id: '2', candidateName: 'Michael Brown', position: 'Product Manager', recommendation: 'Hire' }
  ]);
  const [candidates, setCandidates] = useState<Candidate[]>([]);
  const [feedbackData, setFeedbackData] = useState<InterviewFeedbackData>({
    technicalSkills: 0, problemSolving: 0, communication: 0, cultureFit: 0, strengths: '', weaknesses: '', notes: '', recommendation: 'Hold'
  });
  const [upcomingInterviews, setUpcomingInterviews] = useState<Interview[]>([
    { id: '1', candidate: 'Sarah Johnson', position: 'Senior Software Engineer', time: 'Today, 2:00 PM', type: 'Technical Round', status: 'Scheduled', date: new Date().toISOString() },
    { id: '2', candidate: 'Michael Brown', position: 'Product Manager', time: 'Tomorrow, 11:00 AM', type: 'Behavioral Round', status: 'Scheduled', date: new Date(Date.now() + 86400000).toISOString() },
    { id: '3', candidate: 'Emily Davis', position: 'UX Designer', time: 'Tomorrow, 3:30 PM', type: 'Portfolio Review', status: 'Scheduled', date: new Date(Date.now() + 86400000).toISOString() }
  ]);
  const [pendingFeedbacks, setPendingFeedbacks] = useState<Feedback[]>([
    { id: '1', candidate: 'Sarah Johnson', deadline: 'Today', status: 'Pending' },
    { id: '2', candidate: 'Michael Brown', deadline: 'Tomorrow', status: 'Pending' }
  ]);
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        setIsLoading(true);
        setError(null);
        // (In a workaround, we do not rely on session status; you can replace these fetch calls with your actual endpoints or mock data.)
        const [statsRes, interviewsRes, feedbackRes, pendingRes] = await Promise.all([
          fetch('/api/interviewer/stats'),
          fetch('/api/interviewer/interviews'),
          fetch('/api/interviewer/feedback/recent'),
          fetch('/api/interviewer/feedback/pending'),
        ]);
  ...
  const handleSubmitFeedback = async () => {
    ...
    setError(null);
    try {
      await fetch('/api/interviewer/feedback', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ candidateId: selectedCandidate.id, ...feedbackData }) });
      setCandidates(prev => prev.map(c => (c.id === selectedCandidate.id ? { ...c, feedbackSubmitted: true } : c)));
      handleCloseModal();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'An error occurred');
    } finally {
        ...
        <ErrorBoundary>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {isLoading ? (
              ...
              <>
                <StatCard title="Total Interviews" value={stats.totalInterviews} icon={Users} change="+12% from last month" isPositive={true} />
                <StatCard title="Completed" value={stats.completedInterviews} icon={FileText} change="+8% from last month" isPositive={true} />
                <StatCard title="Upcoming" value={stats.upcomingInterviews} icon={Clock} change="+3 new this week" isPositive={true} />
                <StatCard title="Average Rating" value={stats.averageRating} icon={CalendarIcon} change="+0.2 from last month" isPositive={true} />
              </>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8">
            <div className="lg:col-span-2">
              <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-6">
                ...
                <InterviewCalendar date={date} onDateChange={setDate} interviews={upcomingInterviews} />
              </div>
